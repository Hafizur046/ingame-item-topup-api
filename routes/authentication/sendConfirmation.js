...
function SendConfirmation(User){
    ...
    return async (req, res, next)=>{
        if(req.user){
            try {
                const tempCode = await genRandomNumber(5);
                console.log('Temp Code:', tempCode);
                req.user = await User.findById(req.user._id);
                const mailSent = sendMail(req.body.email, String(tempCode), "Confirmation Email");
                const updatedUser = User.findByIdAndUpdate(req.user._id, {tempCode: tempCode});
